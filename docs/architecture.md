...
| [`collector/`](../services/collector) | ✅ Qt убран целиком, образ 166 → 102 МБ |

**Задача закрыта: Qt в сервисах не осталось.**
...
#### collector после выноса Qt

- **Qt заменён на standalone asio, а не на системные вызовы.** Первый заход был
  на `poll` и `termios` напрямую — работал, но терял кроссплатформенность,
  которая у проекта архитектурное свойство. asio даёт то же самое на Windows и
  macOS; взят standalone, а не через Boost, чтобы не тащить остальной Boost.
  Библиотека заголовочная, в рантайм-образ не попадает.
- `QCoreApplication` → `asio::io_context`, `QMetaObject::invokeMethod` →
  `asio::post`, обработка сигналов → `asio::signal_set` (её обработчик
  выполняется в цикле как обычная работа, поэтому ничего не нужно делать
  async-signal-safe).
- `QTcpServer` → `asio::ip::tcp::acceptor`, `QSerialPort` →
  `asio::serial_port` с теми же шестью параметрами, что в контракте,
  `QHostAddress` → `asio::ip::make_address`. Последнее меняет поведение: адрес
  обязан быть числовым, имя хоста отвергается с внятной ошибкой, а не
  резолвится молча.
- **Потеря на этом пути одна:** у `asio::serial_port` три режима чётности —
  none, odd, even. `PARITY_SPACE` и `PARITY_MARK` из контракта переносимого
  соответствия не имеют и теперь отвергаются с явной ошибкой; в форме настроек
  их больше нет.
- **Кадрирование стало честным.** Qt-версия искала первую закрывающую скобку, и
  вложенный объект `{"a":{"b":1}}` не разбирался никогда. Новый
  [`JsonFramer`](../services/collector/transfer/json_framer.cpp) считает глубину и
  не считает скобки внутри строк.
- **Наблюдатель перестал быть `QObject`**: `MU_ObserverBase` с виртуальным
  `Update` заменён на `std::function`, и класс-обёртка в `main.cpp` исчез.
- **JSON разбирается один раз в `MonitorUnit`** (nlohmann), а не в транспорте:
  транспорты занимаются байтами. Публикуется компактная форма — это не
  косметика, storage пишет запись в строку, и перевод строки внутри тела
  разрезал бы её надвое.
- **Kafka producer переехал в [`libs/kafka`](../libs/kafka)** к консьюмеру: теперь
  общая библиотека покрывает обе стороны топика.
- Ловушка, найденная на живом стеке в промежуточной версии на `poll`: цикл
  откладывал снятие дескриптора до конца итерации, а ядро выдавало тот же номер
  новому сокету при смене порта — и отложенное удаление вычёркивало уже новый
  слушатель. Соединение устанавливалось, но никто его не принимал. В версии на
  asio такого класса ошибок нет: дескрипторами владеет библиотека.
...
### MQTT-транспорт — на рассмотрении, решение не принято

Вопрос возник из «а можно слушать Zigbee-устройства». Напрямую — нет: Zigbee это
радио, для него нужен координатор и стек, разбирающий его бинарный протокол.
Обычный путь — [zigbee2mqtt](https://www.zigbee2mqtt.io/), который говорит с
донглом, а наружу отдаёт JSON в MQTT:

```
zigbee2mqtt/датчик_кухня   {"temperature":21.3,"humidity":45.2,"battery":100}
```

Форма ровно та, которую система уже умеет, — параметры обнаруживаются из данных.
Вопрос только в том, как эти сообщения попадают внутрь.

**Что даст.** Третий транспорт в `oneof TransferSettings` открывает не только
Zigbee: на MQTT говорят Tasmota, ESPHome, Shelly, промышленные шлюзы. В отличие
от нынешних двух, он не требует связи с устройством: collector становится
клиентом брокера, как и они, и переживает пропажу любого из них.

**Чего стоит.** Класс `MqttTransport` на libmosquitto под тем же
[`TransferInterface`](../services/collector/transfer/transfer_interface.h),
вариант в контракте, поля в форме настроек. Плюс одна правка не в транспорте:
сейчас ключ сообщения Kafka — это `collector_id`, и десяток датчиков слился бы в
одну карточку. Нужно, чтобы ключ задавал транспорт, отображая тему в
идентификатор, — иначе `temperature` кухонного датчика смешается с уличным.

**Чем можно обойтись.** Мост в одну строку, без правок в коде:
`mosquitto_sub -t 'zigbee2mqtt/+' | socat - TCP:collector:2323`. Или отдельный
`mqtt-service`, публикующий в Kafka мимо collector'а, — если MQTT-источников
станет много.

**Порядок, если делать.** Транспорт ложится в
[`collector/transfer/`](../services/collector/transfer) — слой, который стоит в
плане на вынос Qt. Разумно сначала переписать слой, потом добавлять в него
транспорт, как это было со `storage-service` и `DataLoad`.

**Что решить:** есть ли реальные источники на MQTT. Пока их нет, задача не
начинается: мост покрывает разовые пробы, а транспорт без устройств — код,
