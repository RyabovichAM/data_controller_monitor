...
Свойства значения — в панели над холстом. Клик по значению в режиме «Выбор»
выделяет его пунктирной рамкой и открывает панель: параметр, подпись, единицы,
размер, цвет, удаление
([`LabelPanel.tsx`](../web-ui/src/schemes/LabelPanel.tsx)). Все эти поля
`Label` были в контракте с первой версии, но из браузера задавался только
параметр.

- **Параметр подсказывается из живого потока, но не навязывается.** Поле
  предлагает ключи, которые сейчас приходят от collector'а схемы; молчащий
  collector тоже имеет параметры, поэтому можно ввести любое имя.
- **Новое значение сразу открывает панель** с курсором в поле параметра — вместо
  `window.prompt`, где имя приходилось вводить вслепую. Значение без параметра
  до config-service не доходит: редактор выделяет его и говорит почему.
- **Пустой размер и пустой цвет означают «по умолчанию»,** а не конкретное
  значение: поле размера можно стереть и набрать заново, а «сброс» цвета
  возвращает цвет темы, а не сегодняшний синий.
- **Строка панели зарезервирована** и без выделения показывает подсказку: панель,
  появляясь на первом клике, сдвигала бы холст под курсором.
- **Рамка меряется по `getBBox` после каждой отрисовки** — ширина текста меняется
  с каждым пришедшим значением.
- **Перетаскивание помнит, за какое место взяли значение;** без этого оно
  прыгало бы якорем под курсор.
- Двойной клик больше ничего не удаляет. Кнопка удаления названа полностью —
  «Удалить значение»: строкой выше стоит «Удалить» самой схемы.
